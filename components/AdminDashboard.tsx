...
interface AdminDashboardProps {
  ...
  onExportDeployJson: () => void; // New prop for exporting deployment JSON
        ...
        <div className="max-w-4xl mx-auto">
          <header className="mb-10">
            ...
            <p className="text-slate-500 dark:text-slate-400">
              ...
              {activeTab === 'database' && 'Xuất dữ liệu cho việc Deploy hoặc Reset hệ thống.'}
            </p>
          </header>
          ...
          {activeTab === 'database' && (
             <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                ...
                {/* Deployment Export Section */}
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-8">
                    <div className="mb-6 flex items-start gap-4">
                       <div className="p-2 bg-purple-100 dark:bg-purple-900/30 text-purple-600 rounded-lg"><UploadCloud className="w-6 h-6" /></div>
                       <div>
                          <h4 className="font-bold text-slate-900 dark:text-white text-lg">Triển khai dữ liệu (Deploy)</h4>
                          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 max-w-xl">
                            Vì đây là web tĩnh (Static Web), để người dùng khác thấy dữ liệu mới, bạn cần xuất file 
                            <code className="bg-slate-100 dark:bg-slate-800 px-1 py-0.5 rounded mx-1 text-teal-600 font-mono">data.json</code> 
                            và đặt vào thư mục <code className="bg-slate-100 dark:bg-slate-800 px-1 py-0.5 rounded mx-1 text-teal-600 font-mono">public</code> 
                            của mã nguồn, sau đó deploy lại.
                          </p>
                       </div>
                    </div>
                    
                    <button 
                        onClick={onExportDeployJson}
                        className="w-full sm:w-auto px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl shadow-md shadow-purple-600/20 transition-all flex items-center justify-center gap-2"
                    >
                        <FileJson className="w-5 h-5" />
                        Tải file data.json (Cho Deploy)
                    </button>
                </div>
                      ...
                      {!showResetConfirm ? (
                          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                              <div>
                                  ...
                                  <p className="text-sm text-slate-500 dark:text-slate-400">Hành động này sẽ xóa vĩnh viễn tất cả các bản ghi hiện có trên trình duyệt này.</p>
                              </div>
                              <button 
                                  type="button"
