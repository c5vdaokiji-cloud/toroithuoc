...
import { parseExcelFile, exportToExcel, loadDataFromUrl } from './services/excelService';
...
const MainApp: React.FC = () => {
  ...
  // --- Initial Data Load (Hybrid: Static JSON + Local DB) ---
  useEffect(() => {
    const initializeData = async () => {
      try {
        // 1. Try to fetch the static 'data.json' file (Deployed Data)
        // We assume the file is named 'data.json' and sits in the root (public folder)
        const remoteItems = await loadDataFromUrl('/data.json');
        
        // 2. Load Local Data (User specific favorites/edits)
        ...
        if (remoteItems.length > 0) {
           // If remote data exists, it takes precedence for structure, 
           // but we try to preserve local "favorites" by matching items.
           finalItems = remoteItems.map(rItem => {
              const localMatch = localItems.find(l => 
                l.stt === rItem.stt && l.name === rItem.name
              );
              return localMatch ? { ...rItem, isFavorite: localMatch.isFavorite } : rItem;
           });
           
           // Update local DB to match remote (so it works offline next time)
           await dbService.saveItems(finalItems);
           source = 'remote';
        } else {
           // Fallback to local DB if no remote file found (or offline)
           finalItems = localItems;
        }

        if (finalItems.length > 0) {
          setItems(finalItems);
          // Removed auto-navigation to allow Landing Page to show on reload
          // setShowLanding(false); 
          
          if (source === 'remote') {
             console.log("Loaded data from deployment file (data.json)");
          } else {
             // Only show toast if we are actually entering the app or if it doesn't disturb the landing page too much
             // For now, let's keep it informative
             console.log(`Restored ${finalItems.length} items from local DB`);
          }
        }
      } catch (err) {
  ...
  const handleUpdateItem = async (originalItem: TradeItem, updatedItem: TradeItem) => {
    try {
      // Find index in the original items array
      // Since we don't have a unique ID, we check by reference or fallback to comparing properties
      const newItems = items.map(item => {
         if (item === originalItem) return updatedItem;
         // Fallback if reference is lost (e.g. after spread)
  ...
  const handleToggleFavorite = async (itemToToggle: TradeItem) => {
    ...
    // Save to DB to persist favorites
  ...
  // Function passed to AdminDashboard to handle JSON export for deployment
  const handleExportDeployJson = () => {
     // We export the full item list
     const success = exportToExcel(items, 'data.json'); 
     // Reusing logic, but actually we need the specific JSON export utility
     // Using a more direct approach here via the service
     import('./services/excelService').then(mod => {
         mod.exportToJsonFile(items, 'data.json');
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300 relative overflow-hidden">
      ...
      {/* Admin Dashboard Page (Full Screen Overlay) */}
      <AdminDashboard 
        isOpen={showAdminDashboard}
        onClose={() => setShowAdminDashboard(false)}
        onUpload={handleFileUpload}
        onResetDatabase={handleResetDatabase}
        onExportDeployJson={handleExportDeployJson} // Pass the handler
          ...
          <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
            ...
            {viewMode === 'list' ? (
                <DataTable 
                  ...
                  onUpdateItem={isAdmin ? handleUpdateItem : undefined} // Only pass update handler if Admin
                />
            ) : (
                <DataGrid 
                  data={paginatedItems} 
