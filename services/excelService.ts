  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const items = parseSheetToItems(sheet);
        resolve(items);
      } catch (error) {
...
// Helper function to process Sheet data (reuse for both File and Google Sheet)
const parseSheetToItems = (sheet: XLSX.WorkSheet): TradeItem[] => {
...
export const fetchGoogleSheetData = async (csvUrl: string): Promise<TradeItem[]> => {
    try {
        ...
        return parseSheetToItems(sheet);
    } catch (error) {
